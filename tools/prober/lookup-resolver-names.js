...
class ENSResolverAnalyzer {
  ...
  async fetchWithRetry(query, variables, retries = MAX_RETRIES) {
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        ...
        this.stats.errors++;
        const base =
          typeof RETRY_DELAY === 'number' ? RETRY_DELAY : parseInt(RETRY_DELAY || '500', 10);
        const backoff = base * Math.pow(2, attempt - 1) + Math.floor(Math.random() * 100);
        console.error(
          `Attempt ${attempt}/${retries} failed: ${error.message}. Retrying in ${backoff}ms`
        );
        if (attempt === retries) {
          throw error;
        }
  ...
  async fetchResolverContracts() {
    ...
    while (true) {
      ...
      process.stdout.write(
        `\rProcessed ${totalProcessed} domains, found ${resolverMap.size} resolvers`
      );
  ...
  async testResolverResolution(resolverAddress, domainName) {
    try {
      ...
      try {
        ...
        console.log(`[PASS] addr() -> ${ethAddress}`);
      } catch (e) {
        console.log(`[FAIL] addr() failed: ${e.message}`);
      }

      try {
        const contenthash = await resolver.contenthash(node);
        console.log(`[PASS] contenthash() -> ${contenthash || 'none'}`);
      } catch (e) {
        console.log(`[FAIL] contenthash() failed: ${e.message}`);
      }
      ...
      for (const key of textKeys) {
        try {
          const value = await resolver.text(node, key);
          if (value) {
            console.log(`[PASS] text("${key}") -> ${value}`);
          }
        } catch (e) {
  ...
  analyzeENSResolverType(resolver, domains) {
    const ensPatterns = {
      ...
      isOfficialENS:
        /^0x[a-f0-9]{40}$/i.test(resolver.id) && domains.some((d) => d.name.endsWith('.eth')),
    ...
    return {
      type: ensPatterns.publicResolver
        ? 'public-resolver'
        : ensPatterns.isOfficialENS
          ? 'official-ens'
          : ensPatterns.customResolver
            ? 'custom'
            : 'unknown',
  ...
  formatCSV(results) {
    const headers =
      'Resolver,DomainCount,EthDomains,CustomDomains,Type,HasTexts,HasContenthash,AvgNameLength,UniqueOwners,OnChainCapabilities\n';
    const rows = results
      .map(
        (r) =>
  ...
  printStats() {
    ...
    console.log(
      `Cache hit ratio: ${((this.stats.cachehits / (this.stats.queriesExecuted + this.stats.cachehits)) * 100).toFixed(1)}%`
    );
  }
  ...
  async showStats() {
    try {
      ...
      console.log(
        `Most Popular Resolver: ${stats.mostPopularResolver?.split('-')[0]} (${stats.maxDomains} domains)`
      );
      console.log(`SubGraph Queries Executed: ${this.stats.queriesExecuted}`);
      console.log(
        `Cache Hit Ratio: ${((this.stats.cachehits / (this.stats.queriesExecuted + this.stats.cachehits)) * 100).toFixed(1)}%`
    ...
    try {
      ...
      try {
        ...
        while (maxBatches > 0) {
          ...
          process.stdout.write(
            `\rFound ${resolverAddresses.size} resolver addresses via direct query`
          );
        ...
        while (true) {
          ...
          process.stdout.write(
            `\rProcessed ${totalProcessed} domains, found ${resolverAddresses.size} resolver addresses`
          );
  ...
  async analyze() {
    try {
        ...
        if (domains.length > 0) {
          ...
          if (resolver.sampleDomains.length > 0) {
            ...
            if (onChainData && onChainData.resolver === resolverAddress) {
              console.log(`  [PASS] Domain ${sampleDomain.name} uses resolver ${resolverAddress}`);
              ...
              console.log(`  [PASS] Direct resolution - address: ${content?.ethAddress || 'none'}`);
              console.log(
                `  [PASS] Direct resolution - content hash: ${content?.contenthash || 'none'}`
              );
              console.log(
                `  [PASS] Direct resolution - text records: ${Object.keys(content?.textRecords || {}).length} found`
              );
            } else {
              console.log(
                `  [FAIL] Domain ${sampleDomain.name} resolver mismatch: expected ${resolverAddress}, got ${onChainData?.resolver || 'none'}`
              );
            }
          }
          ...
          const onChainSamples = await Promise.all(
            resolver.sampleDomains.slice(0, 3).map(async (sample) => {
              ...
              const content = onChainData
                ? await this.resolveENSContent(sample.name, onChainData.resolver)
                : null;
              return {
                name: sample.name,
                onChain: onChainData,
          ...
          results.push({
            ...
            contractDomains: domains.map((domain) => ({
              ...
              expiryDate: domain.expiryDate
                ? new Date(parseInt(domain.expiryDate) * 1000).toISOString()
                : null,
      ...
      results.sort((a, b) => {
        ...
        return ensScoreDiff !== 0
          ? ensScoreDiff
          : b.contractDomains.length - a.contractDomains.length;
      });
...
async function showHelp() {
  ...
  console.log('\nExamples:');
  console.log(
    '  node lookup-resolver-names.js addresses           # List all resolver addresses (direct query)'
  );
  console.log(
    '  node lookup-resolver-names.js analyze             # Full analysis with domain counts'
  );
    ...
    switch (command) {
      ...
      case 'check-resolvers':
        if (args.length === 0) {
          ...
          console.log(
            'Example: node lookup-resolver-names.js check-resolvers 0x231b0Ee14048e9dCcD1d247744d114a4EB5E8E63'
          );
          process.exit(1);
        }
        {
...
async function lookupNamesByResolver(resolverAddress, options = {}) {
  // Set environment variables based on options
  if (options.output) {
    process.env.OUTPUT_FILE = options.output;
  }
  if (options.format) {
    process.env.OUTPUT_FORMAT = options.format;
  }

  // Run the main function with check-resolvers command
  process.argv = ['node', 'lookup-resolver-names.js', 'check-resolvers', resolverAddress];

  try {
    await main();
    return { success: true, message: 'Lookup completed' };
  } catch (error) {
