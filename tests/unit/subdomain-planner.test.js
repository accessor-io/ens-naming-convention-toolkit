...
describe('Subdomain Planner', () => {
  ...
  describe('Plan generation', () => {
    ...
    test('should validate plan structure', () => {
      ...
      const hasRequiredFields = requiredFields.every(
        (field) => plan.hasOwnProperty(field) && plan[field]
      );
  ...
  describe('File operations', () => {
    test('should save plan to file', () => {
      ...
      expect(fs.writeFileSync).toHaveBeenCalledWith(filename, JSON.stringify(plan, null, 2));
    });
  ...
  describe('Domain validation', () => {
    test('should validate ENS domain format', () => {
      const validDomains = ['test.eth', 'sub.test.eth', 'v3.uniswap.amm.eth'];

      const invalidDomains = ['invalid', 'test.', '.eth', 'test..eth'];
