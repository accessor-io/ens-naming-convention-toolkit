...
class ENSecurityAnalyzer {
  ...
  displayReport(results) {
    ...
    const gradeColor = overall.grade === 'A' ? chalk.green :
                      overall.grade === 'B' ? chalk.blue :
    ...
    Object.entries(checks).forEach(([checkName, check]) => {
      ...
      const statusColor = check.status === 'PASS' ? chalk.green :
                         ...
                         check.status === 'FAIL' ? chalk.red : chalk.gray;
    ...
    if (recommendations && recommendations.length > 0) {
      ...
      recommendations.forEach((rec, index) => {
        ...
        console.log(`\n${index + 1}. ${priorityIcon} ${chalk.bold(rec.action)}`);
    ...
    console.log(chalk.bold('\n📋 Summary:'));
    console.log(`Critical Issues: ${criticalIssues > 0 ? chalk.red(criticalIssues) : chalk.green('0')}`);
    console.log(`High Priority Issues: ${highIssues > 0 ? chalk.yellow(highIssues) : chalk.green('0')}`);
    console.log(`Recommendations: ${recommendations.length}`);

    if (criticalIssues === 0 && highIssues === 0) {
      console.log(chalk.green('✅ Domain appears to have good security posture'));
    } else {
      console.log(chalk.red('⚠️  Domain has security issues that should be addressed'));
    }
  }
}
